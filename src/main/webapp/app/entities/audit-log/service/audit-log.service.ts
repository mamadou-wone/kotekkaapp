...
type RestOf<T extends IAuditLog | NewAuditLog> = Omit<T, 'performedDate'> & {
...
export class AuditLogService {
  ...
  protected convertDateFromClient<T extends IAuditLog | NewAuditLog | PartialUpdateAuditLog>(auditLog: T): RestOf<T> {
    return {
      ...auditLog,
      performedDate: auditLog.performedDate?.toJSON() ?? null,
    };
  }
  ...
  protected convertDateFromServer(restAuditLog: RestAuditLog): IAuditLog {
    return {
      ...restAuditLog,
      performedDate: restAuditLog.performedDate ? dayjs(restAuditLog.performedDate) : undefined,
    };
  }
